...
package com.oakeel.ejb.entityAndEao.check;
 ...
 */
@Entity
public class CheckEntity extends EntityRoot {
    ...
    @Column(precision=12,scale=2) //精度为12位，小数点位数为2位
    private BigDecimal actualPrincipal=BigDecimal.ZERO;//本金
    @Column(precision=12,scale=2) //精度为12位，小数点位数为2位
    private BigDecimal actualInterest=BigDecimal.ZERO;//利息
        
    @Column(precision=12,scale=2) //精度为12位，小数点位数为2位
    private BigDecimal actualSubTatal=BigDecimal.ZERO;//小计
    @Temporal(TemporalType.TIMESTAMP)
    private Date actualSettlementTime;//结算时间
    @Column(precision=12,scale=2) //精度为12位，小数点位数为2位
    private BigDecimal actualBeforeBalance=BigDecimal.ZERO;//期初余额
    @Column(precision=12,scale=2) //精度为12位，小数点位数为2位
    private BigDecimal actualAfterBalance=BigDecimal.ZERO;//期末余额
