...
//        int num=ptpSessionBean.getIssueBondLocal().getCurrBond().getCopiesNum();
//        int baseAmount=ptpSessionBean.getIssueBondLocal().getCurrBond().getBaseAmountEnum().getBaseValue();
//        allAmount=num*baseAmount;
    ...
    public void caculateModel() {
        ...
        //根据输入和选择的还款模型计算还款列表
        try {
            setExpenseEntitys(repayModelCaculateLocal.caculateRepayModel(bond3.getRepayModelEnum(), bond3.getRepayCycle(), getAllAmount(), bond3.getYearRate().multiply(BigDecimal.valueOf(0.01), MathContext.DECIMAL32), bond3.getRepayCycleNumber(), bond3.getStartDate()));
