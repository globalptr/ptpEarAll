...
public class RandomValidateCode {
    ...
    public void getRandcode()   {
        ...
        try
        {
            try (FileOutputStream fo = new FileOutputStream("MyOut.jpg")) {
                ImageIO.write(image, "JPEG", out);//将内存中的图片通过流动形式输出到客户端
                ImageIO.write(image, "JPEG", fo);//将内存中的图片通过流动形式输出到客户端
            } //将内存中的图片通过流动形式输出到客户端

        }
        catch(IOException ex)
        {
            System.out.println("写入图片缓存出现错误");
        }
        ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(out.toByteArray());
        setDsc(new DefaultStreamedContent(byteArrayInputStream,"image/JPEG"));
    ...
    /**
     * @param randomStr the randomStr to set
     */
